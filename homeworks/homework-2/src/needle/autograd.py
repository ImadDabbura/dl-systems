...
class Tensor(Value):
    ...
    @data.setter
    def data(self, value):
        assert isinstance(value, Tensor), print(type(value))
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for out_tensor in reverse_topo_order:
        ...
        if out_tensor.op:
            partial_adjoints = out_tensor.op.gradient_as_tuple(
                out_grad, out_tensor
            )
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for output_node in node_list:
        for input_node in topo_sort_dfs(output_node, visited, topo_list):
            topo_list.append(input_node)
    return topo_list
    ### END YOUR SOLUTION


def topo_sort_dfs(node, visited, topo_list):
    """Post-order DFS"""
    ### BEGIN YOUR SOLUTION
    for input_node in node.inputs:
        if input_node not in visited:
            for other in topo_sort_dfs(input_node, visited, topo_list):
                yield other
    visited.append(node)
